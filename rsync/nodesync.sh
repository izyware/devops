...
REMOTEHOMEDIR=`ssh $SSH_OPTIONS_IDENTITY_FILE $USERNAME@$MACHINEADDR pwd`;
...
ssh $SSH_OPTIONS_IDENTITY_FILE $USERNAME@$MACHINEADDR $REMOTECMD
echo [$SCRIPTNAME] rsync local:$LOCALDIR to remote:$REMOTEDIR
rsync -av -e "ssh $SSH_OPTIONS_IDENTITY_FILE" --exclude="node_modules/*" --exclude=".git/*" --exclude=".DS_Store" $LOCALDIR/ $USERNAME@$MACHINEADDR:$REMOTEDIR
if [ -z "$APPNAME" ]
then
      ...
      echo [$SCRIPTNAME] remotecmd $REMOTECMD
      ssh -t $SSH_OPTIONS_IDENTITY_FILE $USERNAME@$MACHINEADDR $REMOTECMD
fi
